      ...
      <div class="modal-content border-secondary bg-dark text-light">
        ...
        <div class="modal-body bg-body-tertiary">
          <div class="alert alert-info border-secondary py-2 small">
            ...
            {{ analysis.conflicts.length }} naming conflicts found.
          </div>

          <div v-if="analysis.conflicts.length > 0">
            <h6 class="text-danger border-bottom border-secondary pb-2 mb-3">Resolve Conflicts by Name</h6>
            ...
            <div v-for="(conflict, index) in analysis.conflicts" :key="index" class="card bg-dark border-danger mb-3">
              <div class="card-header border-danger py-2 d-flex justify-content-between align-items-center">
                <span class="fw-bold text-warning">Name: {{ conflict.name }}</span>
                ...
                <table class="table table-sm table-dark mb-0 align-middle">
                  <thead>
                    <tr>
                      ...
                      <th class="w-50">Imported CSV State</th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr>
                      <td class="ps-3 border-end border-secondary">
                        <div class="text-muted small">IP: {{ conflict.existing.ip }}</div>
                        <div class="text-muted small">Type: {{ conflict.existing.manufacturer }}</div>
                      </td>
                      <td>
                        <div class="fw-bold text-warning">IP: {{ conflict.incoming.ip }}</div>
                        <div class="text-muted small">Type: {{ conflict.incoming.manufacturer }}</div>
                      </td>
                    </tr>
                  </tbody>
                </table>
              ...
              <div class="card-footer border-danger bg-dark py-2">
                <div class="btn-group w-100 shadow-sm">
                  ...
                  <label class="btn btn-outline-secondary" :for="`skip-${index}`">Keep Current</label>

                  <input type="radio" class="btn-check" :name="`res-${index}`" :id="`overwrite-${index}`" value="overwrite" v-model="conflict.resolution">
                  <label class="btn btn-outline-danger" :for="`overwrite-${index}`">Overwrite with CSV</label>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="modal-footer border-secondary justify-content-between bg-dark">
          ...
          <button type="button" class="btn btn-info fw-bold" @click="submitMerge" :disabled="processing">
            {{ processing ? 'Applying...' : 'Execute Merge' }}
          </button>
        </div>
      </div>
    </div>
...
const submitMerge = async () => {
  ...
  try {
    const payload = {
      ...
      resolved_conflicts: props.analysis.conflicts.filter(c => c.resolution === 'overwrite')
    }
    const response = await axios.post('/api/v1/system/import/apply', payload)
    
    if (response.data.success) {
      store.addToast('Configuration merged successfully.')
      await store.fetchSystemData()
      emit('merged')
    }
  } catch (error) {
    store.addToast('Failed to apply merge.', 'danger')
  } finally {
